...
- **`/workflows:compound` to `/lfg`** - Automatically capture learnings at the end of each /lfg workflow

---

## [2.26.5+fork.6] - 2026-01-19

### Removed

- **Compound Engineered badge** - Removed badge and checklist item from `/workflows:work` PR template
...
- **`/workflows:compound` scope selection** - Always show all 4 scope options (Team, Personal, Global, Skip). The "(Recommended)" label moves based on learning type, but options are never hidden.
